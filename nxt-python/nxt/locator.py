...
import traceback, configparser as ConfigParser, os
...
def find_bricks(host=None, name=None, silent=False, method=Method()):
    ...
    if method.usb:
        try:
            from .usbsock import find_bricks
            methods_available += 1
            socks = find_bricks(host, name)
            for s in socks:
                yield s
        except ImportError:
            import sys
            if not silent: print("USB module unavailable, not searching there")

    if method.bluetooth:
        try:
            from .bluesock import find_bricks, bluetooth
            methods_available += 1
            try:
                socks = find_bricks(host, name)
                for s in socks:
                    yield s
            except (bluetooth.BluetoothError, IOError): #for cases such as no adapter, bluetooth throws IOError, not BluetoothError
                pass
        except ImportError:
            import sys
            if not silent: print("Bluetooth module unavailable, not searching there")

    if method.fantom:
        try:
            from .fantomsock import find_bricks
            methods_available += 1
            if method.fantomusb:
                usbsocks = find_bricks(host, name, False)
                for s in usbsocks:
                    yield s
            if method.fantombt:
                btsocks = find_bricks(host, name, True)
                for s in btsocks:
                    yield s
        except ImportError:
            import sys
...
def server_brick(host, port = 2727):
    from .ipsock import IpSock
    sock = IpSock(host, port)
    return sock.connect()
